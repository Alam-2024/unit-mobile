import { Pk3ShortTitlesTypes } from "@/../interfaces/pk3/pkTypes";

export const TeamWorkInActionThirdData: Pk3ShortTitlesTypes = {
  unitTitle: "Team Work in Action",
  TheBigIdea: {
    description:
      "Playing with others helps us build friendships, share, and have fun as a team.",
  },
  EssentialQuestions: {
    questions: [
      "What is the difference between balance and static balance?",
      "What is the difference between dynamic balance and static balance?",
    ],
  },
  LearningGoals: {
    GrossMotorSkills: [
      "Games with physical exercise",
      "Fair play and showing respect for opponents",
    ],
    LocomotorSkills: [],
    NonLocomotorSkills: [
      "Teamwork",
      "Communication",
      "Sportsmanship",
      "Fair play",
      "Respect for opponents",
    ],
    ObjectControlSkills: [],
  },
  InstructionAndLearningPlan: {
    disciplinarySkills: [
      "Participating in basic chasing games, like 'tag.'",
      "Learning to follow simple game instructions, such as stop and go.",
      "Walking or running alongside peers during group activities.",
      "Rolling balls to a partner or simple relay movements.",
    ],
    knowledgeAndVocabulary: [
      "Tag",
      "Stop and go",
      "Walking or running alongside peers",
      "Rolling balls to a partner",
    ],
  },
};
