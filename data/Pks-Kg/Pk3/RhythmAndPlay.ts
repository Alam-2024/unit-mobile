import { Pk3ShortTitlesTypes } from "@/interfaces/pk3/pkTypes";

export const RhythmAndPlayThirdData: Pk3ShortTitlesTypes = {
  unitTitle: "Rhythm and Play",
  TheBigIdea: {
    description:
      "Moving to music and stories helps us express ourselves and enjoy being active.",
  },
  EssentialQuestions: {
    questions: [
      "What is the difference between balance and static balance?",
      "What is the difference between dynamic balance and static balance?",
    ],
  },
  LearningGoals: {
    LocomotorSkills: [],
    NonLocomotorSkills: ["swinging", "swaying"],
    GrossMotorSkills: ["Music, rhythm, and movement activities"],
    ObjectControlSkills: [],
  },
  InstructionAndLearningPlan: {
    disciplinarySkills: [
      "Clapping and stomping to simple rhythms.",
      "Moving freely to music with basic steps, like marching or swaying.",
      "Jumping in response to changes in music tempo.",
      "Imitating animal movements during imaginative play.",
    ],
    knowledgeAndVocabulary: [
      "Clapping",
      "Stomping",
      "Marching",
      "Swaying",
      "Jumping",
      "Imitating animal movements",
    ],
  },
};
